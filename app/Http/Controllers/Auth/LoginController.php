    ...
    if( Request::isMethod('post') && Session::token() == Input::get('_token') ){
      ...
      if($validator->fails()){
        ...
        if(!empty($data) && isset($data->password) && isset($data->id)){
          ...
          if(Hash::check(Input::get('admin_login_password'), $password) && Hash::check(Input::get('admin_login_password'), $data->password) && isset($get_user_role['user_role_slug']) && ($get_user_role['user_role_slug'] == 'administrator') || ($get_user_role['user_role_slug'] == 'vendor')){
